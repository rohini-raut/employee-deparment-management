...
('Amit','Jain',13,52000,'2024-05-01',' amit.jain@example.com');
...
select * from Employees
where DateOfJoining = '2024-3-01' and Salary>55000;
...
-- department.
select min(DateOfJoining) from Employees
...
select count(EmployeeID), DateOfJoining from Employees
where DateOfJoining = '2024-01-01'
group by EmployeeID;
...
where Salary between 90000 and 70000
group by  DepartmentID;
...
-- manager.
select HeadOfDepartment,count(DepartmentID) from Departments
...
select Location , min(AnnualBudget),max(AnnualBudget) from Departments
group by Location;
...
select DepartmentName,Location, concat(DepartmentName, ' ' ,Location ) as Departmentaddress from Departments;
-- Situation -3: To prepare a personalized email greeting, concatenate "Hello " with the employee's
-- name and a comma.
select concat('Hello', FirstName,'a',LastName) as emailgreeting from Employees;
-- Situation -4: For creating a unique employee ID, concatenate the department ID and employee
-- number.
select distinct EmployeeID ,concat(EmployeeID, '' ,DepartmentID) from Employees;
...
select concat(FirstName, '' ,LastName, '' ,PhoneNumber) as Contact_list from Departments;
...
select FirstName,LastName,DepartmentID , concat(FirstName,'',LastName,'',DepartmentID) as infromation from Employees;
-- Situation -8: For sending out a personalized notification, concatenate the employee's department 
-- with their position.
select concat(DepartmentName, '',Position) from Deparments;
-- Situation -9: To display the employee's work status, concatenate their job title and current       
-- project.
select concat(Status, '' ,JObTitle) from Deparments;
-- Situation -10: For an annual report, concatenate the year and employee’s first name to create a
-- unique report reference code. # not
select concat(year, '' ,FirstName) from Employees;
...
update Employees
set JobTiltle ='Senior Developer'
...
delete from Employees
where hired = 'North' and Empoyment<2023;
...
PARTITION p_2020_and_after VALUES LESS
THAN MAXVALUE
);            
